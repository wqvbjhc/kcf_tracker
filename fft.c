	...
	for(m=1;m<=M;m++)
	{
		...
		for(l=1;l<=lb;l++)
		{
			...
			for(n=l-1;n<N-1;n=n+la) 
			{
				...
				c_mul(in[lc],wn,&t);//t = f[lc] * wn
				c_sub(in[n],t,&(in[lc]));//f[lc] = f[n] - f[lc] * Wnr
				c_plus(in[n],t,&(in[n]));//f[n] = f[n] + f[lc] * Wnr
			}
		}
	}
}
...
void fft_2D(int mLen,int nLen,int M,int N,complexdouble *A_In,int flag)
{
	...
	if (flag == DXT_INVERSE)
	{
		...
		for(i=0; i<len; i++)
		{
			(p->imag) *= -1;    //A_In[i].image = -A_In[i].image;
			p++;
		}
	}	
	...
	//printf("\n%f\n%f\n",A_In[0].real,A_In[0].imag);
	//printf("\n%f\n%f\n",A_In[1].real,A_In[1].imag);
	//printf("\n%f\n%f\n",A_In[2].real,A_In[2].imag);
	#if 0
	fp=fopen("A_In.txt","w");
	for(i=0; i<mLen; i++)
	{
		for(j=0; j<nLen; j++)
		{
			fprintf(fp,"%f,%f ",A_In[i*nLen+j].real,A_In[i*nLen+j].imag);
		}
		fprintf(fp,"\n");
	}
	fclose(fp);
